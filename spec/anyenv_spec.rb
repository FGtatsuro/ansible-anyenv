...
describe command("su -l -s /bin/bash -c 'anyenv install ndenv'"), :if => ['debian'].include?(os[:family]) do
  its(:exit_status) { should eq 0 }
end

describe command("su -l -s /bin/bash -c 'which ndenv'"), :if => ['debian'].include?(os[:family]) do
  its(:exit_status) { should eq 0 }
end
